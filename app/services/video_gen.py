...
def generate_ideas(topic_name: str, theme_prompt: str | None, existing: list[str],
    ...
    if content_format == "long":
        ...
        prompt = (
            ...
            "of the theme. "
            "Prefer high-retention hook patterns: doubt frames (Is X worth it or just pain?), "
            "pattern interrupts (Stop using X — do Y instead), direct questions with stakes "
            "(Why does your X fail in production?), speed hooks (X in 60 seconds), or "
            "comparisons (X vs Y: which survives production?). Avoid vague or generic titles. "
            "Do NOT repeat or closely paraphrase any of these existing titles:\n"
            f"{avoid or '(none yet)'}\n\n"
            "Return ONLY the titles, one per line, no numbering, no bullets, no commentary."
        )
