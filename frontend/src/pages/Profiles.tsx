...
function ProfileEditor({ profile, onClose }: { profile: Partial<RenderProfile>; onClose: () => void }) {
  ...
  const save = () => {
    const body = { name, channel_id: channelId ? Number(channelId) : null, params };
    if (profile.id) m.updateProfile.mutate({ id: profile.id, body: { name, params } }, { onSuccess: onClose });
        ...
        <div className="max-h-[52vh] overflow-y-auto pr-1">
          ...
          {GROUPS.map((grp) => (
            <div key={grp.title} className="mb-4">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                {grp.fields.filter((f) => opts.fields[f]).map((field) => (
              ...
              <div key={p.id} className="panel p-5 card-hover">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <div className="label mt-1">{channelName(p.channel_id)} · {Object.keys(params).length} settings</div>
                  </div>
                  <div className="flex gap-2">
